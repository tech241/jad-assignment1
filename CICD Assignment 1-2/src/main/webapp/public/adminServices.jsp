...
<body>
	...
	<main>
		...
		<div class="container">
			...
			<div class="search-by">
				...
				<form>
					...
					<label for="category">Category</label> <select name="category"
						...
						<option value="<%=rsCat2.getString("cat_id")%>"
							<%=rsCat2.getString("cat_id").equals(request.getParameter("category")) ? "selected" : ""%>><%=rsCat2.getString("cat_name")%></option>
			...
			<button onclick="location.href = 'adminCreateService.jsp'">Create
				service</button>
			<button onclick="location.href = 'adminCreateCategory.jsp'">Create
				category</button>
			...
			<div class="category-list">
				...
				try {
					...
					catRs = catStmt.executeQuery("SELECT * FROM service_category ORDER BY cat_id;");
				} catch (Exception e) {
				...
				<div class="category-card">
					<span><strong><%=catRs.getString("cat_name")%></strong></span>

					<button
						onclick="location.href='adminDeleteCategory.jsp?catId=<%=catRs.getString("cat_id")%>'"
						class="dangerous-button">Delete</button>
				</div>
			...
			<div class="search-results" id="search-results">
				...
				try {
					...
					stmtCat3 = conn3
					.prepareStatement("SELECT * FROM service s JOIN service_category c ON s.cat_id = c.cat_id" + where + ";");
				...
				<div class="service-card">
					...
					<span> <strong><%=rsCat3.getString("service_name")%></strong><br>
						...
						<a
						href="services.jsp?service_id=<%=rsCat3.getString("service_id")%>">View
							page</a><br>

						<div class="action-buttons">
							<button
								onclick="location.href = 'adminEditService.jsp?serviceId=<%=rsCat3.getString("service_id")%>'"
								class="secondary-button">Edit</button>
							<button
								onclick="location.href = 'adminDeleteService.jsp?serviceId=<%=rsCat3.getString("service_id")%>'"
								class="dangerous-button">Delete</button>
						</div>
					</span>
				</div>
